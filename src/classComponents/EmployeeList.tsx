...
export class EmployeeListCC extends Component<IProps, IState> {
  ...
  constructor(props: IProps) {
    ...
    this.handleAssignShiftToEmployee =
      this.handleAssignShiftToEmployee.bind(this);
  }
  ...
  async handleAssignShiftToEmployee(
  ...
  render(): ReactNode {
    ...
    return (
      <>
        <h2>Employee List</h2>
        {this.state.employees.map((employee: IEmployee) => (
          <div key={`employee-list-${employee.id}`}>
            <span style={{ marginRight: '10px' }}>
              {employee.firstName} {employee.lastName} -{' '}
              {employee.shift?.name ?? 'Keine Schicht'}
            </span>
            <select
              value={employee.shift?.id}
              onChange={(e) => this.handleAssignShiftToEmployee(e, employee)}
            >
              <option value="">Bitte auswählen</option>
              {this.state.shifts.map((shift: IShift) => (
                <option
                  key={`select-shift-option-${shift.id}`}
                  value={shift.id}
                >
                  {shift.name}
                </option>
